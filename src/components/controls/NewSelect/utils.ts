...
export const isOptionShape = (
  item: OptionShape | string | null,
): item is OptionShape =>
...
export const joinOptions = ({
  ...
  return options.reduce(
    (acc: Array<ReactNode | string>, option: OptionShape, index: number) => {
  ...
  useEffect(() => {
    ...
    if (open && list && styleTarget) {
      ...
      if (visibleOptions < list.children.length) {
        // Добавляем половинку
        height += Math.ceil(
          optionsNodes[optionsNodes.length - 1].clientHeight / 2,
        );
      }
